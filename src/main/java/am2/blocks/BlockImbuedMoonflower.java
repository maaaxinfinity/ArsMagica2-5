...
public class BlockImbuedMoonflower extends BlockBush
{
    ...
    protected BlockImbuedMoonflower()
    {
    ...
    public BlockImbuedMoonflower setUnlocalizedNameAndID(String name){
        setBlockName(name);
    ...
    public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int metadata, int fortune)
    {
        ...
        if (metadata < 3) succeed = false; // fully grown
        int phase = BossSpawnHelper.instance.getMoonPhaseProxiedProperly(world.provider.getWorldTime());
        if (phase != 0) succeed = false; // full moon

        if (succeed)
        {
            ret.add(new ItemStack(ItemsCommonProxy.itemOre, 1, ItemsCommonProxy.itemOre.META_IMBUEDMOONFLOWER));
            ret.add(new ItemStack(ItemsCommonProxy.imbuedMoonflowerSeeds));
        }
